...
import GardenRoomDocumentsView from '../../components/customer/GardenRoomDocumentsView';
import OutdoorKitchenDocumentsView from '../../components/customer/OutdoorKitchenDocumentsView';
    ...
    <div className="space-y-4 max-w-5xl w-full font-body text-[#4A4A43]">
      ...
      <div className="bg-[#FAF7F2] border border-[#E4DED4] p-2.5 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 text-xs shadow-xs">
        <span className="font-bold text-primary font-heading px-1 flex items-center gap-1.5">
          ...
          <span>Testing Documents View Switcher:</span>
        </span>
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          >
            Garden Room / Poolhouse (Documents)
          </button>
        </div>
      </div>
      ...
      {/* Render 1-to-1 Views */}
      {isGardenRoom ? (
        <GardenRoomDocumentsView project={projectForChild} />
      ) : (
        <OutdoorKitchenDocumentsView project={projectForChild} />
      )}
    </div>
  );
}
